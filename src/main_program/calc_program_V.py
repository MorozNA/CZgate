...
plt.plot(tau_array * 1e9, 1 - np.array(fidelity0), color='purple', linestyle='dashed', label=r'$T \approx 0 K$')
plt.plot(tau_array * 1e9, 1 - np.array(fidelity1), color='red', label='$T = 1 \mu K$')
plt.plot(tau_array * 1e9, 1 - np.array(fidelity2), color='green', label='$T = 5 \mu K$')
plt.plot(tau_array * 1e9, 1 - np.array(fidelity3), color='blue', label='$T = 10 \mu K$')
plt.plot(tau_array * 1e9, 1 - np.array(f_expected), 'k--', label='eq. (2.20)')

plt.title(r'homogeneous spatial profiles')
plt.ylabel('Infidelity')
plt.xlabel(r'$2 \cdot \tau$, ns')
plt.legend()
plt.show()
