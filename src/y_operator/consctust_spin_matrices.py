...
def get_OM_eff(t, om, delta_rydberg=None):
...
def get_V1(t, om, delta_rydberg=None):
    return V_matrix1
...
def get_V2(t, om, delta_rydberg=None):
    return V_matrix2


def get_W0z(t, om, delta_rydberg=None):
    const_w0z = W_INT_CONSTANT * get_OM_eff(t, om, delta_rydberg).conj() * 1j * HBAR / Z_ast / p0z
...
def get_Wz(t, om, delta_rydberg=None):
    const_wz = W_INT_CONSTANT * (HBAR ** 2) / (p0z ** 2)
    Wz_matrix = np.zeros((3, 3), dtype=complex)
    Wz_matrix[0, 0] = -DELTA_a
    Wz_matrix[1, 1] = -DELTA_b
    Wz_matrix[1, 2] = get_OM_eff(t, om, delta_rydberg).conj() / 2
    Wz_matrix[2, 1] = get_OM_eff(t, om, delta_rydberg) / 2
