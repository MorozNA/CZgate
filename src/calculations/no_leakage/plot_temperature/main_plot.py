...
from src.calculations_no_leakage.initial_parameters import get_rho_T0, n
from src.calculations_no_leakage.plot_fideliry_vs_iter.main_code import rho_TB_1_list
from src.y_operator.params import HBAR, OM_small


temperatures = [get_temperature(np.diag(rho)) for rho in rho_TB_1_list]
...
current_iter = 0
rho = rho_TB_1_list[current_iter]
probs = rho.real
...
ax1.set_ylim(0, np.amax(rho_TB_1_list[0]) + 0.05)
# mask = probs > 1e-10
# if np.sum(mask) >= 2:
