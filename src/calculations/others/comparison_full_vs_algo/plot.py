import numpy as np
from src.calculations.comparison_full_vs_algo.comparison import fidelity, fidelity_spin, fidelity_vib, fidelity_vib_diag
from src.calculations.comparison_full_vs_algo.comparison import purity_spin_full, purity_spin_alg, fidelity_decomp
from src.calculations.comparison_full_vs_algo.comparison import iterations


x_step = iterations // 10
path = 'data4/off_diagonal/'
...
plt.plot(np.arange(iterations) + 1, fidelity_vib, 'o-',
         ...
         markerfacecolor='white',  # White fill for markers
         markeredgewidth=1.5,      # Marker edge thickness
         label=r'full $\rho^{(vib)}$')

# plt.plot(np.arange(iterations) + 1, fidelity_vib_diag, 'o-',
#          color='red',  # Use a professional blue color
#          markersize=6,     # Slightly larger markers
#          linewidth=1.5,     # Slightly thicker line
#          markerfacecolor='white',  # White fill for markers
#          markeredgewidth=1.5,      # Marker edge thickness
#          label=r'diagonal $\rho^{(vib)}$')
