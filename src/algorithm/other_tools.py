...
def get_rho_T0(params: YOperatorDerived, T):
    ...
    En = HBAR * params.OM_small * (np.arange(params.n) + 0.5)
...
def generalized_fidelity(rho, sigma):
    ...
    # return np.clip(fidelity.real, 0.0, 1.0)   # Ensure numerical stability (clip to [0, 1] due to floating point errors)
